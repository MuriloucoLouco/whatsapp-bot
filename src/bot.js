...
module.exports = class Bot extends EventEmitter {
  ...
  async send_message(message) {
    await this.page.evaluate((message) => {
      let e = new InputEvent('input', {bubbles: true});
  ...
  async get_last_message() {
    let message_data = await this.page.evaluate(() => {
      const message_list = document.getElementsByClassName('message-in');
      const message_node = message_list[message_list.length - 1];
